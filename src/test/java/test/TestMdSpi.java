...
import thostmduserapi.CThostFtdcDepthMarketDataField;
import thostmduserapi.CThostFtdcMdApi;
import thostmduserapi.CThostFtdcMdSpi;
import thostmduserapi.CThostFtdcReqUserLoginField;
import thostmduserapi.CThostFtdcRspInfoField;
import thostmduserapi.CThostFtdcRspUserLoginField;
import thostmduserapi.CThostFtdcSpecificInstrumentField;
import thostmduserapi.CThostFtdcUserLogoutField;

/**
 * Copyright (c) 2011-2013, z16304607@163.com Created with IntelliJ IDEA. User:
 * trade Date: 13-4-12 Time: 下午12:49 To change this template use File | Settings
 * | File Templates.
 */
public final class TestMdSpi extends CThostFtdcMdSpi {
	 ...
	 */
	@Virtual(6)
	public void OnRspSubMarketData(Pointer<CThostFtdcSpecificInstrumentField> pSpecificInstrument,
			Pointer<CThostFtdcRspInfoField> pRspInfo, int nRequestID, boolean bIsLast) {
	 ...
	 */
	@Virtual(7)
	public void OnRspUnSubMarketData(Pointer<CThostFtdcSpecificInstrumentField> pSpecificInstrument,
			Pointer<CThostFtdcRspInfoField> pRspInfo, int nRequestID, boolean bIsLast) {
	...
	private String ToString_CThostFtdcDepthMarketDataField(CThostFtdcDepthMarketDataField data) {
		return String.format("%s %s.%03d %f %d %f %d %f %d", data.InstrumentID().getCString(),
				data.UpdateTime().getCString(), data.UpdateMillisec(), data.LastPrice(), data.Volume(),
				data.AskPrice1(), data.AskVolume1(), data.BidPrice1(), data.BidVolume1());
	}
}
